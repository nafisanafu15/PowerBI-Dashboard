...
function applyFilters(){
  ...
  if(rangeFrom) data = data.filter(r => r._start && r._start >= rangeFrom);
  if(rangeTo) data = data.filter(r => r._start && r._start <= rangeTo);
...
function updateTable(data){
  ...
  else if(currentSort === 'offer') rows.sort((a,b) => String(a.course).localeCompare(String(b.course)));
  ...
  rows.forEach(r => {
    ...
    tr.innerHTML = `<td>${r.status}</td><td>${r.periods}</td><td>${r.course}</td>`;
